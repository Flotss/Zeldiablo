...
public class LabyDessin implements DessinJeu {
    ...
    /**
     * indique si les images on été chargées ou non
     */
    ...
    public void dessinerJeu(Jeu jeu, Canvas canvas) {
        ...
        // si les images n'ont pas été chargées, on les charge et on indique qu'elles ont été chargées
        if(!IMAGE_CHARGE){
        ...
        if (!dernierUpdateFait) {
            ...
            for (int i = 0; i < labyJeu.getLaby().getMurs().length; i++) {
                for (int j = 0; j < labyJeu.getLaby().getMurs()[i].length; j++) {
                    ...
                    }else if (labyJeu.getLaby().getMur(i, j)) {
                        gc.drawImage(imageMur, i * 60, j * 60);
                    }else if (labyJeu.getLaby().getCaisse(i, j)) {
